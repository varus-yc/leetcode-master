...
![image](https://user-images.githubusercontent.com/70749838/127802714-f534fcb4-c391-4514-98dd-122d6f9a0ce7.png)
    ...
    int removeElement(vector<int>& nums, int val) {
        ...
        for (int i = 0; i < size; i++) {
            if (nums[i] == val) { // 发现需要移除的元素，就将数组集体向前移动一位
                ...
                i--; // 因为下表i以后的数值都向前移动了一位，所以i也向前移动一位，防止漏删
                size--; // 此时数组的大小-1
            }
        }
        return size;
...
有两个指针，前面一个后面一个，前面的用于搜索需要删除的值，当遇到需要删除的值时，前指针直接跳过，后面的指针不动，当遇到正常值时，两个指针都进行移动，并修改慢指针的值。最后只需输出慢指针的索引即可。
...
public:
    int removeElement(vector<int>& nums, int val) {
        ...
        for (int fastIndex = 0; fastIndex < nums.size(); fastIndex++) {
            ...
            //若值不等，则将此处fastIndex的值赋给slowIndex处
                //slowIndex自增
