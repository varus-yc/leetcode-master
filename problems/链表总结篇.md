...
迭代法使用两个指针pre与cur，首先pre指向NULL，cur指向头结点，{暂时保存temp = cur->next节点，cur->next指向pre，然后调整pre = cur，cur = temp}，然后重复以上步骤直到cur为NULL，此时所有节点反转。返回反转后的头结点pre。
...
假设链表长度为L，需要删除倒数第N个节点，则一个指针需要移动的步数为（L-N）；定义两个指针fast、slow指向头结点，首先让fast移动N步，则余下要走的步数为（L-N）步，此时fast与slow同时移动，每次移动一步，当fast移动到链表末尾时，slow移动了L-N步，到达需要删除的节点。

可以结合虚拟头结点的方法，处理实际头结点的逻辑；此时fast需要移动的步数为（N+1）步，余下要走的步数为（L-N-1）步；slow移动了（L-N-1）步，到达需要删除的节点的前一个结点。
...
定义两个指针a，b分别指向长链表1与短链表2，a走过链表1-2的长度差值，与b对齐后，每次向后移动一步，必定相交于链表的交点。
...
定义快慢指针，一个走两步，一个走一步，证明链表有环，其次根据快慢指针走过的路径进行数学计算，证明即可。
