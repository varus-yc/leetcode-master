...
void insert(struct unordered_set** hashtable, int ikey,int ival) {
    //插入的时候需要检查是否已经在哈希表中，此题在前面已经检查过，故去掉加快运行时间
    //struct unordered_set* it = find(hashtable, ikey);
    //if (it != NULL) return;
    ...
    struct unordered_set* it = find(hashtable,ikey);
    if (it == NULL) {
        struct unordered_set* tmp = malloc(sizeof(struct unordered_set));
...
int* twoSum(int* nums, int numsSize, int target, int* returnSize){
    ...
    for(int j = 0; j < numsSize;j++){
        ...
        struct unordered_set* it = find(&set1,temp);   
        if( it!= NULL){
            res[0] = it->val;
            res[1] = j;
            *returnSize = 2;
